...
import type { OnboardingActionResult } from "@/app/(auth)/onboarding/actions";
import { useRouter } from "next/navigation"; // <-- action의 반환 타입 import
...
export function OnboardingForm({
  ...
  onSubmit: (values: OnboardingPayload) => Promise<OnboardingActionResult>;
}) {
  ...
  const checkAvailability = useCallback(
    async (type: "nickname" | "blog-slug", value: string) => {
      if (!value) return;
      const setter =
        type === "nickname" ? setCheckingNickname : setCheckingSlug;
      setter(true);
      try {
        const res = await fetch(
          `${apiBase}/api/v1/me/availability/${type}?value=${encodeURIComponent(value)}`,
          {
            credentials: "include",
          },
        );
        ...
        toast.error(
          error instanceof Error ? error.message : "중복 확인 중 오류 발생",
        );
      } finally {
        setter(false);
      }
  ...
  const handleProfileImageChange = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setValue("profile_image_url", result.secure_url, {
          shouldValidate: true,
        });
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit(async (values) => {
        ...
        const toastId = toast.loading("온보딩 정보를 저장하는 중입니다...");
        const result = await onSubmit(payload);
        toast.dismiss(toastId);

        if (result.success) {
          toast.success("온보딩이 완료되었습니다! 잠시 후 이동합니다...");
          // 서버가 알려준 URL로 클라이언트에서 리다이렉트
          router.push(result.redirectUrl);
          // router.refresh()를 호출하여 서버 컴포넌트 데이터를 새로고침
          router.refresh();
        } else {
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <Input
          id="blog_name"
          {...register("blog_name")}
          placeholder="예: DevAll 블로그"
        />
        {errors.blog_name && (
          <p className="text-sm text-red-600">{errors.blog_name.message}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          URL은 영문 소문자, 숫자, 하이픈만 사용할 수 있습니다.
        </p>
        {errors.blog_slug && (
          <p className="text-sm text-red-600">{errors.blog_slug.message}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {watch("profile_image_url") && (
          <p className="text-xs text-muted-foreground">
            현재 이미지: {watch("profile_image_url")}
          </p>
        )}
