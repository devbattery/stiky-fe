    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input placeholder="예: backend" {...register('category')} />
          {errors.category && <p className="text-sm text-red-600">{errors.category.message}</p>}
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">공개 상태</label>
      ...
      <div className="space-y-2">
        ...
        <Textarea rows={3} placeholder="글에 대한 간단한 설명을 입력하세요" {...register('description')} />
        {errors.description && <p className="text-sm text-red-600">{errors.description.message}</p>}
      </div>

      <div className="space-y-2">
        <label className="text-sm font-medium text-foreground">태그 (쉼표로 구분)</label>
        <Input placeholder="예: fastapi, nextjs, devall" {...register('tags')} />
        {errors.tags && <p className="text-sm text-red-600">{errors.tags.message}</p>}
      </div>

      <div className="space-y-2">
        <label className="text-sm font-medium text-foreground">표지 이미지</label>
        <input type="file" accept="image/*" onChange={handleUpload} className="text-sm" />
        {watch('thumbnail_url') && (
          <p className="text-xs text-muted-foreground">현재 이미지: {watch('thumbnail_url')}</p>
        )}
        {errors.thumbnail_url && <p className="text-sm text-red-600">{errors.thumbnail_url.message}</p>}
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        {errors.content_md && <p className="text-sm text-red-600">{errors.content_md.message}</p>}
      </div>
